...
def add_query_dic(word_dic,is_need=False):
	...
	query_file = "../data_set/rob04-title/rob04.title.krovetz.txt"
...
def get_wordDic_Embedding(dim = 50):
	...
	build_doc_word_dic = read_trainSet("../pre-process/rob04-title/final_train_file.txt","../pre-process/rob04-title/final_test_file.txt")
...
def divide_document_features_Set():
	...
	f = open("../pre-process/rob04-title/document_features_tfidf_top50.txt")
	out1 = open("../pre-process/rob04-title/train_file.txt",'w')
	out2 = open("../pre-process/rob04-title/test_file.txt",'w')

	qid = []
	for line in f.readlines():
		linelist = line.split('\t')
		if int(linelist[0]) not in qid:
			qid.append(int(linelist[0]))
	f.close()
	f = open("../pre-process/rob04-title/document_features_tfidf_top50.txt")
	# print(qid)
...
def get_queryContent_label(data_dir, outfile, test = True):
	...
	# data_dir = "data_set/clueweb09B-title/train_data.txt"
	query_dir = "../data_set/rob04-title/rob04.title.krovetz.txt"
	# label_file = "data_set/clueweb09B-title/prels.catB.1-50"
	label_file = "../data_set/rob04-title/qrels.rob04.txt"
	# outfile = "data_set/clueweb09B-title/corpus_query1_50.txt"
	f1 = open(data_dir)
	f2 = open(query_dir)
	f3 = open(label_file)
	...
	if test:
		...
		counter = data.groupby("query_ID").apply(lambda data: sum(data["query_ID"] <= 1000))
		...
		query_is_qid = counter[counter>0].index
		df = data[data["query_ID"].isin(query_is_qid)]
		test_data,train_data = train_test_split(df,test_size=0.0)
...
def load():
	...
	divide_document_features_Set()
	get_queryContent_label("../pre-process/rob04-title/test_file.txt","../pre-process/rob04-title/final_test_file.txt")
	get_queryContent_label("../pre-process/rob04-title/train_file.txt","../pre-process/rob04-title/final_train_file.txt",False)
	data_train = "../pre-process/rob04-title/final_train_file.txt"
	data_test = "../pre-process/rob04-title/final_test_file.txt"
